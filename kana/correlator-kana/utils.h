...
inline char* getFileDirectory(char* filepath) {
...
inline char** splitString(const char* str, const char delim) {
	int count = 0;
	const char* tmp = str;
...
inline void changeFileExt(char* path, const char* ext) {
	char* dotpos = strrchr(path, '.');
	strcpy(dotpos, ext);
}

inline long double readUnixTime(char* filename) {
...
inline int isFile(struct dirent* entry, char* dir) {
...
inline int isKanaFile(struct dirent* entry, char* dir, char* station) {
...
inline unsigned long getFileSize(char* filePath) {

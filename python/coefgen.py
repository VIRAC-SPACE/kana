...
def main():
    ...
    json_out = {"Stations": stations, "Start_time": Date(setup["start_time"]).get_unix(),
                "End_time": Date(setup["end_time"]).get_unix(), "Object": obj, "F0(MHz)": F0, "Fsf(MHz)": Fsf,
                "F(kHz)": F, "Fsn(Hz)": Fsn, "Fv(Hz)": Fv, "A0": A[0], "A1": A[1], "A2": A[2], "A3": A[3]}
